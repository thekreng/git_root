     ...
     public string displayErrors(byte ec)
        ...
        void ColorOut(string input, string type, PB_Base PB_IN)
        {
            ...
            if (PB_IN != null)
            {
                ...
                ColorOutResponseSsaps(input, type, PB_IN.SSAPS);
                ColorOutReqestSsaps(input, type, PB_IN.SSAPS);
...
#region Testing DATA and func
        ...
        private void DataReceivedHandlerCPU(object sender, SerialDataReceivedEventArgs e)
        {
            if (_continue == true)
            {
                try
                {
                    ...
                    _serialPortSLAVE.Write(transfer_arr_CPU, 0, transfer_arr_CPU.Length);
                }
                    ...
                    if (BYT == (byte)FRAME_TYPE.FRAME_TYPE_SD2 && INbox.Length > i + 1)
                    {
                        ...
                        if (end < INbox.Length)
                        {
                            if (BYT == INbox[i + 3])
                            {
                                ...
                                if (FindFrame[7] == 0x34)//REMOVE DATA UNIT FROM ARRAY 
                                {
        ...
        {
            string databyte = _serialPortSLAVE.ReadExisting();
            MessageBox.Show(databyte);
        }
    }
}
